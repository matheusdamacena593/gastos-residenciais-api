...
export default function TotaisPessoas() {
  const [loading, setLoading] = useState(true);
  const [errorMessages, setErrorMessages] = useState<string[]>([]);
  const [data, setData] = useState<RelatorioTotaisPessoasResponse | null>(null);
  ...
  async function carregar() {
    ...
    try {
      const res = await relatoriosApiService.totaisPessoas(page, pageSize);
      setData(res);
    } catch (err: any) {
  ...
  async function baixarPdfPessoas() {
    setDownloadingPdf(true);
    try {
      const file = await relatoriosApiService.pdfTotaisPessoas();
  ...
  const pessoasPage = data?.pessoas;
  const pessoas = pessoasPage?.items ?? [];
  const totalItems = pessoasPage?.totalItems ?? 0;
  const totalPages = pessoasPage?.totalPages ?? 1;

  const moeda = (v: number) =>
    Number(v).toLocaleString("pt-BR", { style: "currency", currency: "BRL" });

  return (
    <section className="totaisPessoasPage">
      <header className="totaisPessoasHeader">
        <div>
          <h1 className="totaisPessoasTitle">Totais por Pessoas</h1>
          <p className="totaisPessoasSubtitle">
            Visualize receitas, despesas e saldo agrupados por pessoa.
          </p>
        </div>

        <div className="totaisPessoasHeaderActions">
          <Button
            text="Exportar PDF"
        ...
        >
          ...
          >
            ...
            <p>
              ...
              <strong>relatório de totais por pessoas</strong> em PDF. Deseja
              continuar?
            </p>
      ...
      <CrudTable<RelatorioTotaisPessoaItem>
        loading={loading}
        items={pessoas}
        getRowKey={(p) => p.id}
        ...
        headerRight={null}
        columns={[
          { header: "Pessoa", accessor: "nome", mobileLabel: "Pessoa" },
          { header: "Idade", accessor: "idade", mobileLabel: "Idade" },
          {
            header: "Receitas",
            mobileLabel: "Receitas",
            className: "colMoney",
            render: (p) => moeda(p.totalReceitas),
          },
          {
            header: "Despesas",
            mobileLabel: "Despesas",
            className: "colMoney",
            render: (p) => moeda(p.totalDespesas),
          },
          {
            header: "Saldo",
            mobileLabel: "Saldo",
        ...
        emptyText="Ainda não há totais por pessoas."
        pagination={
          pessoasPage
            ? {
                page,
